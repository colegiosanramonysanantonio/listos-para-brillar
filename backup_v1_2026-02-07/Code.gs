...
function getStreak(ss, curso, grupo, alumno) {
  ...
  // 1. Buscar Hoja
  let sheet = ss.getSheetByName(cursoNorm); // Intento directo "1º"
  ...
  if (!sheet) {
    ...
    for (let s of sheets) {
      ...
      // Si el nombre de la hoja contiene "1º" y el grupo (si no es ÚNICO)
      const searchName = cursoNorm.toUpperCase();
      if (sName.includes(searchName)) {
        // Encontró algo como "1º" o "CLASE 1º"
        sheet = s;
        break;
      }
    }
  ...
  if (!sheet) {
    return ContentService.createTextOutput(JSON.stringify({ 
      streak: 0, 
      error: `Hoja no encontrada para curso ${cursoNorm}`,
      lastDate: strDate
    })).setMimeType(ContentService.MimeType.JSON);
  }
  ...
  for (let d = ultimoDiaMarcado; d >= 1; d--) {
      ...
      // Lógica de racha:
      if (esCheck) {
          // Si está marcado, SIEMPRE suma, sea finde o no
          streak++;
          diasRacha.push(d);
      } else {
          // Si NO está marcado, vemos si es lectivo
          // Usamos hora 12:00 para evitar problemas de huso horario
          ...
          if (esFinDeSemana) {
               // DEBUG: Guardar info de que saltamos un finde
               // diasRacha.push(`(Skip ${d} ${diaSemana===0?'Dom':'Sab'})`);
               continue;
          } else {
  ...
  try {
    ...
    if (data.action) {
      if (data.action === 'resetCompetition') {
        ...
        // 2. Limpiar Checkboxes en hojas de clase
        ...
        sheets.forEach(sh => {
          ...
          if (/^\d/.test(name)) { // Si empieza por dígito
             ...
             if (lastRow > 1) {
               // Borramos el rango de checkboxes (Col B a AF, a partir de la fila 2)
               // Column B es 2, Column AF es 32. Total 31 columnas.
               sh.getRange(2, 2, lastRow - 1, 31).clearContent();
             }
          }
        });
    ...
    const now = new Date();
    const timezone = ss.getSpreadsheetTimeZone();
    const fechaStr = Utilities.formatDate(now, timezone, "yyyy-MM-dd");
